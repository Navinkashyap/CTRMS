...
export default function Currency() {
  ...
  const handleEditClick = (currency) => {
    ...
    setFormData({ name: currency.name, status: currency.status });
    setIsModalOpen(true);
  };
      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium tracking-wide flex items-center gap-2">
              Global Financial Units & Exchange
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-slate-200 rounded-[2rem] shadow-2xl shadow-slate-200/50 p-2 overflow-hidden">
            ...
            <table className="w-full text-left text-[14px] border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[11px]">Currency</th>
                  ...
                  <tr key={currency.id} className="group hover:bg-blue-50/40 transition-all duration-300 ease-out cursor-default">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-slate-800 group-hover:text-blue-700 transition-colors tracking-tight uppercase">{currency.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-bold tracking-wide border ${currency.status === 'Active'
                          ? 'bg-emerald-50 text-emerald-600 border-emerald-100'
                          : 'bg-rose-50 text-rose-600 border-rose-100'
                        }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${currency.status === 'Active' ? 'bg-emerald-500' : 'bg-rose-500'} animate-pulse`} />
                        {currency.status}
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
          ...
          <div className="px-8 py-5 flex items-center justify-between border-t border-slate-50 text-xs font-bold text-slate-600 tracking-wider uppercase">
            <span>Showing {filteredCurrencies.length} of {currencies.length} entries</span>
            ...
            <div className="relative px-8 py-8">
              <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 text-sm font-medium">Register global trading units.</p>
                </div>
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2 group">
                    <label className="text-xs font-black text-slate-600 uppercase tracking-widest pl-1">Currency Code</label>
                    <input
