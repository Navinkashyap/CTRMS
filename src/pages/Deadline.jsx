...
export default function Deadline() {
  ...
  const filteredDeadlines = useMemo(() => {
    return deadlines.filter(d =>
      ...
      d.rating.toString().includes(searchQuery)
    );
  ...
  const handleAddClick = () => {
    ...
    setFormData({ name: '', rating: 1, status: 'Active' });
    setIsModalOpen(true);
  };

  const handleEditClick = (deadline) => {
    setEditingDeadline(deadline);
    setFormData({ name: deadline.name, rating: deadline.rating, status: deadline.status });
    setIsModalOpen(true);
  };
      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium tracking-wide flex items-center gap-2">
              Time Performance & Delivery Metrics
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase tracking-tighter">
                {filteredDeadlines.length} Options
              </span>
            </p>
          </div>
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-slate-200 rounded-[2rem] shadow-2xl shadow-slate-200/50 p-2 overflow-hidden">
            ...
            <table className="w-full text-left text-[14px] border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[11px]">Deadline Name</th>
                  ...
                  <tr key={deadline.id} className="group hover:bg-blue-50/40 transition-all duration-300 ease-out cursor-default">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-slate-800 group-hover:text-blue-700 transition-colors tracking-tight">{deadline.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold border ${deadline.rating >= 4 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                          deadline.rating >= 3 ? 'bg-blue-50 text-blue-600 border-blue-100' :
                            'bg-amber-50 text-amber-600 border-amber-100'
                        }`}>
                        Scale {deadline.rating}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-bold tracking-wide border ${deadline.status === 'Active'
                          ? 'bg-emerald-50 text-emerald-600 border-emerald-100'
                          : 'bg-rose-50 text-rose-600 border-rose-100'
                        }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${deadline.status === 'Active' ? 'bg-emerald-500' : 'bg-rose-500'} animate-pulse`} />
                        {deadline.status}
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
          ...
          <div className="px-8 py-5 flex items-center justify-between border-t border-slate-50 text-xs font-bold text-slate-600 tracking-wider uppercase">
            <span>Showing {filteredDeadlines.length} of {deadlines.length} entries</span>
            ...
            <div className="relative px-8 py-8">
              <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 text-sm font-medium">Record delivery expectations.</p>
                </div>
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2 group">
                    <label className="text-xs font-black text-slate-600 uppercase tracking-widest pl-1">Deadline Name</label>
                    <input
                      ...
                      placeholder="e.g. Delivered ASAP"
                    />
                  </div>
