...
const qualityData = [
  { id: 1, type: 'Below Average', rating: 1, status: 'Active' },
  { id: 2, type: 'Average', rating: 2, status: 'Active' },
  { id: 3, type: 'Good', rating: 3, status: 'Active' },
  { id: 4, type: 'Better', rating: 4, status: 'Active' },
  { id: 5, type: 'Best', rating: 5, status: 'Active' },
];

export default function QuiltyRate() {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (editingQuality) {
      setQualities(qualities.map(q => q.id === editingQuality.id ? { ...q, ...formData } : q));
      showNotification(`Updated ${formData.type} successfully!`);
    } else {
      const newId = qualities.length > 0 ? Math.max(...qualities.map(q => q.id)) + 1 : 1;
      setQualities([...qualities, { id: newId, ...formData }]);
            ...
            <table className="w-full text-left text-[14px] border-collapse min-w-[600px]">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredQualities.map((quality, index) => (
                  <tr key={quality.id} className="group hover:bg-blue-50/40 transition-all duration-300 ease-out cursor-default">
                    ...
                    <td className="px-6 py-5">
                       <div className="flex items-center gap-1">
                          ...
                          <span className="ml-2 font-mono text-xs font-bold text-slate-400 group-hover:text-amber-600 transition-colors">({quality.rating})</span>
                       </div>
                    </td>
                    <td className="px-6 py-5">
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2 group">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Quality Type</label>
                    <input 
